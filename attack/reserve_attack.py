...
class XJTUUser(object):
    ...
    def login(self):
        ...
        r=self.session.get('http://rg.lib.xjtu.edu.cn:8080/bxusr/link.jsp?uid='+self.config['username']+'&cn=%E9%83%AD%E7%8E%8B%E6%87%BF&employeeNumber='+self.config['person_fake_id']+'&depId=%E7%94%B5%E5%AD%90%E4%B8%8E%E4%BF%A1%E6%81%AF%E5%AD%A6%E9%83%A8&mobile')#&email=867718012@qq.com去掉了
        #print(r.text)
        #---------------------登陆成功------------------------
        ...
        att=self.session.get('http://rg.lib.xjtu.edu.cn:8010/my/')
        print(att.text)
        while(1):
            cancel(self)
            time.sleep(2)
